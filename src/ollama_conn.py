...
async def writing(response):
    full_response = ""
    try:
        thinking = asyncio.create_task(think(response.message))
        messages = await get_messages(response.message, True)
        
        async for part in chat(messages, context.llama_default_model):
...
async def think(message, timeout=999):
    try:
...
async def chat(messages, model=None, milliseconds=1000):
    if model is None:
...
async def generate(content, model=None):
    if model is None:
        model = context.llama_default_model
    sb = io.StringIO()
    t = datetime.datetime.now()
